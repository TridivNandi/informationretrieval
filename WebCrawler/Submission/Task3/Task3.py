...
def web_spider(seed_url):
    ...
    next_depth = []
    depth = 1
    while pages_to_crawl and len(pages_crawled) < 1000 and depth <= MAXIMUM_DEPTH:
...
#main method
def main():
    print "The crawler is running with seed url as " + "https://en.wikipedia.org/wiki/Solar_power"
    #calling the web_spider method with the given seed. It returns a list of
    #crawled_urls
    my_list = web_spider("https://en.wikipedia.org/wiki/Solar_power")
    #printing the list of crawled urls
    print my_list
    #printing the dictionary which maps the file names with their corresponding URL
